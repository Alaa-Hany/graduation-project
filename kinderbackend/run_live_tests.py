...
def run_tests():
    ...
    test(
        ...
        token=parent_token,
        expect_status=402,
    )  # 402 = CHILD_LIMIT_REACHED on FREE plan
    ...
    test(
        ...
        body={
            ...
            "age": 6,
            "parent_email": f"nobody{ts}@gmail.com",
        },
        expect_status=404,
    )
    ...
    if not rate_hit:
        print(
            "  ⚠️   [rate limit] Did not trigger 429 after 7 attempts (may need more requests or different window)"
        )
